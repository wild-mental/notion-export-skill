---
name: notion-export
description: Resource-complete Notion backup/export using script-based recursive Notion Export zip automation. Use when the user asks to back up, export, download, archive, or recursively save Notion pages, especially with images, files, attachments, resources, teaching materials, token_v2, file_token, Notion CLI, ntn, file:// image refs, subpages, or empty signedUrls. Always prefer the token_v2 + file_token export scripts over per-file getSignedFileUrls.
---
...
| 위치 | 경로 |
|------|------|
| 프로젝트 | `.agents/skills/notion-export/SKILL.md` |
| 개인 | `~/.agents/skills/notion-export/SKILL.md` |

수동 호출: `/skills` 또는 `$notion-export`. 스킬 변경이 반영되지 않으면 Codex를 재시작한다.
