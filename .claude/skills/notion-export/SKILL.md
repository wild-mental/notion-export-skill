---
name: notion-export
description: Resource-complete Notion backup/export using script-based recursive Notion Export zip automation.
when_to_use: Use when the user asks to back up, export, download, archive, or recursively save Notion pages, especially with images, files, attachments, resources, teaching materials, token_v2, file_token, Notion CLI, ntn, file:// image refs, subpages, or empty signedUrls. Always prefer the token_v2 + file_token export scripts over per-file getSignedFileUrls.
---
...
수동 호출: `/notion-export`. 기존 스킬 수정은 세션 중 live 반영된다. 세션 시작 후 새 top-level `.claude/skills/`는 Claude Code 재시작이 필요할 수 있다.
