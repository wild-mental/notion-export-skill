---
name: notion-export
description: Resource-complete Notion backup/export using script-based recursive Notion Export zip automation, capturing images, files, and attachments via token_v2 + file_token.
when_to_use: Use when the user asks to back up, export, download, archive, or recursively save Notion pages — especially with images, files, attachments, resources, or teaching materials — or mentions token_v2, file_token, Notion export zip, file:// image refs, subpages, or empty signedUrls. Always prefer the token_v2 + file_token export scripts over per-file getSignedFileUrls.
---
...
| 위치 | 경로 |
|------|------|
| 프로젝트 | `.claude/skills/notion-export/SKILL.md` |
| 개인 | `~/.claude/skills/notion-export/SKILL.md` |

수동 호출: `/notion-export`. 기존 스킬 디렉터리 수정은 세션 중 live 반영된다. 세션 시작 후
새로 만든 top-level `.claude/skills/`는 Claude Code 재시작이 필요할 수 있다.
